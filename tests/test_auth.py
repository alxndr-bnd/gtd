...
def test_email_signup_normalizes_address(client, mail, login):
    ...
    assert client.get("/api/me").json() == {"name": "alice", "email": "alice@example.com", "tg": False, "google": False}
...
def test_google_signup_and_bad_token(client, google):
    ...
    assert client.get("/api/me").json() == {"name": "Alice", "email": "alice@example.com", "tg": False, "google": True}
